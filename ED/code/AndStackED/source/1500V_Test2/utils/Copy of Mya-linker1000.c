...
typedef struct { 
  ...
  signed char CriticalLevel500  __attribute__((__packed__)) __attribute__ ((aligned (1))),//1   196   (197) 
    CriticalLevel12  __attribute__((__packed__)) __attribute__ ((aligned (1)));      //    1    197   (198)
  ...
  INT16  oc_ramp_level       __attribute__((__packed__)) __attribute__ ((aligned (1)));//  2    232   (234)
...
ParameterBlock page8K __attribute__((__packed__)) __attribute__ ((aligned (1))) = {// 8K Parameter Block
  ...
  0xFFFF, // prepAddr
    // rep_step, ov_startup, t_superhot, t_hot, t_cold, fallback_time, network cnst1, network_cnst_2,  oc_protection
        25,          140,         110,    100,     90,     10,              7,             11,           60,
    //oc_step,   oc_ramp_level (dis),  ov_step, shunt, max_off,
       10,          -1000,               10,      20,      1,
...
ParameterBlock page __attribute__((__packed__)) __attribute__ ((aligned (1))) = {// Main image PB 
  ...
  //  Vout  0         Pin  1         Iout   2         Vin  3         Text 4          Iin2  5         Iout+ 6
  { {27.664, -3575}, {25.083, 0.0}, {7.767, -573.0}, {24.767, 0.0}, {0.435, 625.0}, {7.857, -592 }, {8.037, -545} },
  { 'A', 'N', 'D', 'S',  0,  'l', 'a', 'r', 'A', 'u', 'g', 'u', 's', 't',  0,   7 },   // curKey
  { 'A', 'u', 'g', 'u', 's', 't',  0,   7,  'A', 'u', 'g', 'u', 's', 't',  0,   7 },   // curIV 
  9600, // fetFailCount (2) (in ticks)
  500, // fetCurrentThreshold (2) (in mA)
  100, // fetDeltaCurrent (2) (in mA)
  50,    // mA, oscLowCurrentLimitInmA 
  30000, // raw ADC counts, oscDeltaV
  240,   // 120 ms oscOVStartupDelay
  1200,  // 120*5 ms oscAfterStartupDelay
...
{ 
  ...
  f = fopen( strcat( fname, ".hex" ), "w" );  

  if( f == NULL ) 
    {
      printf("%s:%d ERROR: link failed to open output hex file %s\n",
...
static const int ERROR_NO_TXT_DIR = 12;
static const int ERROR_NO_HEX_DIR = 13;
static const int ERROR_TXT_DIR_NAME = 14;
static const int ERROR_HEX_DIR_NAME = 15;
static const int ERROR_ARGUMENT_CONVERSION = 16;
...
int main(int argc, char **argv)
{
  ...
  INT16  oscLowCurrentLimitInmA = 50;    // mA,  
  ...
  for( i = 1; i < argc && retValue == OK; i++ )
	...
	{ 
      ...
      else if( strcmp( argv[ i ], "-txt-dir"   )    == 0 ){ i++; strcpy(txt_dir, argv[ i ]); }
      else if( strcmp( argv[ i ], "-hex-dir"   )    == 0 ){ i++; strcpy(hex_dir, argv[ i ]); }
      ...
      else if( strcmp( argv[i], "-oscLowCurrentOnlyDelay" ) == 0 )
	{
      ...
      else if( strcmp( argv[ i ], "-oc-ramp-level" ) == 0 ){ i++; sscanf( argv[ i ], "%d", &ocRampLevel ); }
      else if( strcmp( argv[ i ], "-output" )  == 0 ){ i++; additionalOutput = argv[ i ]; }
      else if( strcmp( argv[ i ], "-v-dis-radio" ) == 0 ){ i++; sscanf( argv[ i ], "%g", &vShutUp ); }
      else if( strcmp( argv[ i ], "-vin-k" ) == 0 )  { i++; sscanf( argv[ i ], "%g", &vInK ); }
	...
	{ 
      ...
      else if( strcmp( argv[ i ], "-level12" )      == 0 )
	{ 
	  i++; 
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-level12")) == OK)
	    {
	      level12 = (signed char)val; 
	    }
	}
	...
	{  
      ...
      else if( strcmp( argv[ i ], "-treat-last-byte-as-channel" ) == 0){  page.flags |= 0x8; }
	...
	{ 
      ...
      else if( strcmp( argv[ i ], "-pin-off" ) == 0 )          { i++; sscanf( argv[ i ], "%g", &pInOff ); }
      else if( strcmp( argv[ i ], "-vin-off" ) == 0 )          { i++; sscanf( argv[ i ], "%g", &vInOff ); }
      else if( strcmp( argv[ i ], "-pin-k" ) == 0 )            { i++; sscanf( argv[ i ], "%g", &pInK ); }
      
      else if( strcmp( argv[ i ], "-text-k" ) == 0 )           { i++; sscanf( argv[ i ], "%g", &tExtK ); }
      else if( strcmp( argv[ i ], "-text-off" ) == 0 )         { i++; sscanf( argv[ i ], "%g", &tExtOff ); }
      else if( strcmp( argv[ i ], "-tint-k" ) == 0 )           { i++; sscanf( argv[ i ], "%g", &tIntK ); }
      else if( strcmp( argv[ i ], "-tint-off" ) == 0 )         { i++; sscanf( argv[ i ], "%g", &tIntOff ); }
  ...
  else if ( txt_dir[(strlen(txt_dir)-1)] != slash)
    {
      printf("%s:%d ERROR file name for txt_dir does not end with character %c.\n",
	     __FILE__,__LINE__,slash);
      retValue = ERROR_TXT_DIR_NAME;
    }
  ...
  else if ( hex_dir[(strlen(hex_dir)-1)] != slash)
    {
      printf("%s:%d ERROR file name for hex_dir does not end with character %c.\n",
	     __FILE__,__LINE__,slash);
      retValue = ERROR_HEX_DIR_NAME;
    }
      ...
      if( !is_gw )
	{
	  ...
	  if ((mod < 0) || (mod > 1))
	      ...
	      if( dfk0 < 1E6 ) 
		{
	      ...
	      // Only main PB values (page) set from here on
	      if( vWakeUp >= 0 ) 
		...
		page.linearK[0][1]   = vOutOff;
	      if( pInOff  <1E6 ) 
		page.linearK[1][1]   = pInOff;
	      if( vInOff  <1E6 ) 
		page.linearK[3][1]   = vInOff;
		...
		page.linearK[4][1]   = tExtOff;
	      if( tIntK   <1E6 ) 
		page.linearK[5][0]   = tIntK;
	      if( tIntOff <1E6 ) 
		page.linearK[5][1]   = tIntOff;
      
	      if( ocRampLevel > -99999 ) 
		page.oc_ramp_level = (short) ocRampLevel;
	      if( shortCircuitProtection >=0 ) 
		page.shortCircuitProtection = shortCircuitProtection;
      
	      if( vInK > 0 )  
		page.linearK[3][0]   = vInK;
	      if( ! keep_channel )     
