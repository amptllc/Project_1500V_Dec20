...
void usage( char * argv[] )
{
  ...
  printf("-sku  <required,UINT32> set SKU in Parameter Blocks .\n");
...
int convertSKU(char * productID, 
	       ...
	       UINT16 *secondPartOfSKU)
{
  int retValue = ERROR_BAD_SKU; // Set to OK if good
  long productIDLength = strlen(productID);
  const long int expectedLengthOne = 11; // XXXXXXXX-XX 
  const long int expectedLengthTwo = 13; // XXXXXXXX-XXXX
  const char separator[] = "-";
  char * token;

  if ((productIDLength == expectedLengthOne) ||
      (productIDLength == expectedLengthTwo))
    {
      ...
      if (productID[8] == separator[0])
        {
	  printf("%s:%d SKU separator %s found in productID (%s)\n",
		 __FILE__,__LINE__, separator,productID);
	  fprintf(log,"%s:%d SKU separator %s found in productID (%s)\n",
		 __FILE__,__LINE__, separator,productID);
          /* get the first token */
          token = strtok(productID, separator);
          if ( token != NULL )
            {
	      printf("%s:%d first part of SKU is %s\n",
		     __FILE__,__LINE__, token);
	      fprintf(log, "%s:%d first part of SKU is %s\n",
		     __FILE__,__LINE__, token);
              if (strlen(token) == 8)
                {
                  ...
                  if ( token != NULL )
                    {
		      printf("%s:%d second part of SKU is %s\n",
			     __FILE__,__LINE__, token);
		      fprintf(log, "%s:%d second part of SKU is %s\n",
			      __FILE__,__LINE__, token);
		      size_t secondPartLength = strlen(token);
		      printf("%s:%d second part length of SKU %s is %ld\n",
			     __FILE__,__LINE__, token,secondPartLength);
		      
                      if ((secondPartLength == 4) ||
			  (secondPartLength == 2))
                        {
                          unsigned char secondPart[secondPartLength + 1];
                          memset(secondPart,0,secondPartLength + 1);
                          hex_decode(&productID[9], secondPartLength,&secondPart[0]);
                          token = strtok(NULL, separator);
                          if (token == NULL)
			    {
			      retValue = OK;
				...
				{			    
				  tmp1 = secondPart[0];
				  *secondPartOfSKU = ((tmp1 << 8) & 0xFF00);

				  printf("%s:%d (P) Converted SKU %d:%d\n",
					 __FILE__,__LINE__, 
					 *firstPartOfSKU,*secondPartOfSKU);

				  tmp1 = secondPart[1];
				  *secondPartOfSKU |= (tmp1 & 0x00FF);
				}
			      else
				{
				  tmp1 = secondPart[0];
			      ...
			      *secondPartOfSKU |= (tmp1 & 0x00FF);
			      ...
			      printf("%s:%d SKU Converted : Hex Bytes %d:%d\n",
				     __FILE__,__LINE__, 
				     *firstPartOfSKU,*secondPartOfSKU);
			      fprintf(log,"%s:%d SKU Converted : Hex Bytes %d:%d\n",
				      __FILE__,__LINE__, 
				      *firstPartOfSKU,*secondPartOfSKU);
			    }
			  else
			    {
			      printf("%s:%d ERROR: 2nd separator in %s.\n",
				     __FILE__,__LINE__, productID);
			      fprintf(log,"%s:%d ERROR: 2nd separator in %s.\n",
				      __FILE__,__LINE__, productID);
			    }
			}
                      else
                        {
			  printf("%s:%d ERROR: Part 2 of ProductID length incorrect : %s.\n",
				 __FILE__,__LINE__, productID);
			  fprintf(log,"%s:%d ERROR: Part 2 of ProductID length incorrect : %s.\n",
				  __FILE__,__LINE__, productID);
                    ...
                    {
		      printf("%s:%d ERROR: Failed parsing 2nd part of ProductID %s.\n",
			     __FILE__,__LINE__, productID);
		      fprintf(log, "%s:%d ERROR: Failed parsing 2nd part of ProductID %s.\n",
			      __FILE__,__LINE__, productID);
                    }
                }
              else
                {
		  printf("%s:%d ERROR: Part 1 of ProductID %s length incorrect.\n",
			 __FILE__,__LINE__, productID);
		  fprintf(log,"%s:%d ERROR: Part 1 of ProductID %s length incorrect.\n",
			  __FILE__,__LINE__, productID);
                }
            }
            ...
            {
	      printf("%s:%d ERROR: Failed parsing first part of ProductID %s.\n",
		     __FILE__,__LINE__, productID);
	      fprintf(log,"%s:%d ERROR: Failed parsing first part of ProductID %s.\n",
		      __FILE__,__LINE__, productID);
            }
        }
      else
        {
	  printf("%s:%d ERROR: did not find expected separator %s in ProductID %s.\n",
		 __FILE__,__LINE__, separator,productID);
	  fprintf(log,"%s:%d ERROR: did not find expected separator %s in ProductID %s.\n",
		 __FILE__,__LINE__, separator,productID);
        }
    }
  else
    {
      printf("%s:%d ERROR: length of productID (%s) is %ld, expect either 11 or 13\n",
             __FILE__,__LINE__,productID,strlen(productID));
      fprintf(log,"%s:%d ERROR: length of productID (%s) is %ld, expect either 11 or 13\n",
	      __FILE__,__LINE__,productID,strlen(productID));
    }
...
int main(int argc, char **argv)
{
  ...
  for( i = 1; i < argc && retValue == OK; i++ )
	...
	{ 
	  i++;
	  retValue = convertSKU(argv[i],&firstPartOfSKU,&secondPartOfSKU);
	}
      else
	{
