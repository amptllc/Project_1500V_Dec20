...
struct { //{{{
    ...
    float linearK [7][2]    __attribute__((__packed__)) __attribute__ ((aligned (1)));                                                                                  //* 48
/*
...
*/
    ...
    */
    ...
    BYTE   reserved                  __attribute__((__packed__)) __attribute__ ((aligned (1)));  
...
*/
    ...
    (BYTE)0x89,
    0xFFFF,
    // rep dist, ov startup,  superhot, hot, cold, time,   network cnst1,2,  oc protection
    25,          140,         110,     100, 90,   10,       7, 11,           60,
    //oc step,   oc ramp (dis),  ov step, shunt, max off,
    10,          -1000,          10,      20,      1,
    // vin limit, vin turn on, vin turn off
    0,            0,           0,
    0x5555 // was 0xFFFF
};//}}}
int dummy = 0;
...
unsigned char rbunch = 0, gbunch = 0;
    ...
    if( !is_gw ){
        ...
        if( tenKbod != -1 ) page_gw.modemSpeed = tenKbod;
        if( !linkgw( sn, gw_boot, gw_image ) ) exit( 1 );
        if( sn ) outPageGw( mystrcat( mystrcat( txt_dir, sn ), "_gw.txt" )  );  
        else     outPageGw( mystrcat( mystrcat( txt_dir, mac2serial( page_gw.mac ) ), "_gw.txt" )  );
        if( additionalOutput ) copy( additionalOutput, mystrcat( mystrcat( hex_dir, mac2serial( page_gw.mac ) ), "_gw.hex" ) );
    }
    exit( 0 );
}//}}}
