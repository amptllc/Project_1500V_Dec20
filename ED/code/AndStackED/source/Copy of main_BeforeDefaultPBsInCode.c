...
static const UINT16  FWVersionHigh = 7000;
static const UINT16  FWVersionLow = 8;  // 06/29/2017 - changed 7 to 8
static const BYTE    FWSuffixOne = '2'; // 06/29/2017 - changed 1 to 2
static const BYTE    FWSuffixTwo = 'A';
static const BYTE    FWSuffixThree = ' ';
...
#ifdef MidString
const static UINT16 RawADCVCount = 3000; // (90 / 3) * 100
#endif
...
static UINT16 dVOut = 0, vOutMin = 0,
  vOutMax = 0;

/* 06/26/2017 commented out
void kickOsc()
{
  P1_3 = 0;
  oscWhatToWaitFor = SetOscOVStartupDelay;
  ticks[OscEvent] = page.IhvLimitVout;
...
UINT16 safe_vin_16; //, dp; // dp is needed for power dissipation protection
    ...
    // 06/26/2017 - Added code
    // checking vOut against histerisis limit
    // 06/28/2017 Added in temperature protection logic (isSteppedDown)
    if ( (ocLimit1 == FALSE) &&  ((UINT16)adcs16[0] > ((UINT16)page.VhvLimitVout)) )
    {
      atomicChange( OcRamp, 0 ); 
      ocLimit1 = TRUE;
      // Set OC setting to new value (page.IhvLimitVout if ocLimit2 FALSE which has priority
      if (ocLimit2 == FALSE)
      {
        if (isSteppedDown == FALSE)
        {
          if (theOC > (BYTE)page.IhvLimitVout)
          {
            T3CC1 = oc2timer(theOC = (BYTE)page.IhvLimitVout);
          }
          else // 06/29/2017 - added else case
          {
            T3CC1 = oc2timer(theOC);
          }
          netValue = 92;
        }
        else // isSteppedDown == TRUE
        {
          if (theOC > (BYTE)page.IhvLimitVout) // ensure temperature protection has not lowered OC below InvLimitVout Level
          {
            T3CC1 = oc2timer(theOC = (BYTE)page.IhvLimitVout);
            netValue = 112;
          }
      ...
      else
      {
        if (isSteppedDown == FALSE)
        {
          netValue = 105; // ocLimit, ocLimit2 TRUE
        }
        else
        {
          netValue = 125; // ocLimit1, ocLimit2, isSteppedDown all TRUE
        }
      }
    }
    else if ( (ocLimit1 == TRUE) && (((UINT16)adcs16[0]) <= (((UINT16)page.VhvLimitVout) - RawADCVCount)) )
    {
      // set OC to "normal value" (page.oc) if ocLimit2 is not TRUE
      ocLimit1 = FALSE;
      if (ocLimit2 == FALSE)
      {
        if (isSteppedDown == FALSE)
        {
          T3CC1 = oc2timer(theOC = setOC); // 06/29/2017 - took out  = page.oc
          netValue = 50;  // Back to "normal" OC
        }
        else
        {
          netValue = 110; // isSteppedDown TRUE
        }
      }
      else
      {
        if (isSteppedDown == FALSE)
        {
          netValue = 96;  // ocLimit2 TRUE
        }
        else
        {
          netValue = 116;  //ocLimit2, isSteppedDown TRUE
        }
      }
    }
    
    // checking vIn's (vIn1=adcs16[1], vIn2=adcs16[3]) against hysterisous limits (has priority over vOut settings
    if ( (ocLimit2 == FALSE) && ( (((UINT16)adcs16[1]) > ((UINT16)page.VhvLimitVins) ) || 
                                 (((UINT16)adcs16[3])  > ((UINT16)page.VhvLimitVins) ) ) )
    {
      ocLimit2 = TRUE;
      atomicChange( OcRamp, 0 );       
      if (theOC > (BYTE)page.IhvLimitVins) // Simple check if temperature protection has not reduced OC below InvLimitVins limit.
      {
        T3CC1 = oc2timer(theOC = (BYTE)page.IhvLimitVins);
      }
      if (ocLimit1 == TRUE)
      {
        if (isSteppedDown == TRUE)
        {
          netValue = 125; // ocLimit1,ocLimit2,isSteppedDown all TRUE
        }
        else
        {
          netValue = 105; // ocLimit1, ocLimit2 TRUE
        }
      }
      else
      {
    ...
    else if ( (ocLimit2 == TRUE) && ( (((UINT16)adcs16[1]) <= (((UINT16)page.VhvLimitVins) - RawADCVCount )) &&
                                       (((UINT16)adcs16[3]) <= (((UINT16)page.VhvLimitVins) - RawADCVCount)) ) )
    {
      ocLimit2 = FALSE;
      if (ocLimit1 == FALSE)
      {
        if (isSteppedDown == FALSE) // So going "normal"
        {
          T3CC1 = oc2timer(theOC = setOC); // 06/29/2017 - took out = page.oc
          netValue = 50;
        }
        ...
        {
          if (theOC > page.oc_protection)
          {
            T3CC1 = oc2timer(theOC = page.oc_protection);
          }
          netValue = 110; // isSteppedDown = TRUE, temperature protection code will deal with OC setting
        }
      }
      else
      {
        if (isSteppedDown == FALSE) // ocLimit1 TRUE
        {
          T3CC1 = oc2timer(theOC = (BYTE)page.IhvLimitVout);
          netValue = 92;
        }
        else // ocLimit1, isSteppedDown TRUE
        {
          if (theOC > (BYTE)page.IhvLimitVout)
          {
            T3CC1 = oc2timer(theOC = (BYTE)page.IhvLimitVout);
    ...
    #ifdef MidString    
    //if( !ch255 && (safe_vin_16 > switchTo255Lim ) )
    ...
    if (!ch255)
    {
      ...
      else if ((INT16)adcs16[6] < (INT16)page.LowCurrentLimitTo255)
      {
        ...
        netValue = 86;
            ...
            (( safe_vin_16 < switchFrom255Lim ) && ((INT16)adcs[6] >= (INT16)page.LowCurrentLimitTo255)))
    {
        read_page();   
        if( page.is500Always ) 
    ...
    { 
      ...
      if (ocLimit1 == TRUE)
      {
        ...
        {
          if (theOC > page.VhvLimitVout)
          {
            T3CC1 = oc2timer(theOC = page.IhvLimitVins);            
          }
          netValue = 112;
        }
      }
      else if (ocLimit2 == TRUE)
      {
        if (theOC > page.VhvLimitVins) // 06/29/2017 added if clause include oc2timer(
        {
          T3CC1 = oc2timer(theOC = page.IhvLimitVins);
        }
        netValue = 116;
    ...
    if( page.fallback_time && page.fallback_time!=0xFF )
    { // Temperature protection
      ...
      else
      {
        ...
        if( isSteppedDown )  // Some form of temperature protection happening
        {
          if ( t < ((INT16)page.ton_fallback) ) // temperature less than T_COLD
          {
            if ( !P1_4 ) 
              ov_startup();   // module attempt turn on , starting OV seq
#ifdef MidString
            if (ocLimit2 == TRUE) // VhvLimitVins protection has priority over vOut protection, so check first
            {
              //06/29/2017 - addeded if logic for "lesser" oc value
              if (setOC > page.IhvLimitVins)
              {
                T3CC1 = oc2timer(theOC = page.IhvLimitVins);
              }
              else
              ...
              if (ocLimit1 == TRUE)
              {
                netValue = 105;  // isSteppedDown set false at end of clause.
              }
              else
              {
                netValue = 96;
            ...
            else if (ocLimit1 == TRUE)
            {
              // 06/29/2017 - added if logic for "lesser" oc value
              if (setOC > page.IhvLimitVout)
              {
                T3CC1 = oc2timer(theOC = page.IhvLimitVout);
              }
              else
              {
                T3CC1 = oc2timer(theOC = setOC);
              }
              netValue = 92; // isSteppedDown set false at end of clause
            }
            else
            {
              T3CC1 = oc2timer( theOC = setOC );
        ...
        else if ( P1_4 && ( t > ((INT16)page.toff_fallback)) )
        {
          ...
          if (ocLimit1 == TRUE)
          {
            if (ocLimit2 == TRUE)
            {
              netValue = 125; // ocLimit1,ocLimit2,isSteppedDown = TRUE
            }
            else
            {
              netValue = 112;
            }
          } // END if (ocLimit1 == TRUE)
          else if (ocLimit2 == TRUE)
          {
            netValue = 116;
          }
...
/* 06/26/2017 commented out
	    case OscEvent:
		  ...
		  && ( page.LowCurrentLimitTo255 != 0 )
		  && ( page.LowCurrentLimitTo255 != 0xFFFF ) )
		switch( oscWhatToWaitFor )
		  ...
		  case SetOscAfterStartupDelay:
		    oscWhatToWaitFor = SetOscLowCurrentOnlyDelay;
		    ticks[OscEvent] = page.IhvLimitVins;
		    P1_3 = page.mpp;
		    break;
		  case SetOscOVStartupDelay:
			...
			ticks[OscEvent]= page.VhvLimitVins;
		      }
		    break;
		  default:
		    oscWhatToWaitFor = SetOscLowCurrentOnlyDelay;
...
*/    
    ...
    // 06/26/2017 commented out oscWhatToWaitFor = SetOscLowCurrentOnlyDelay;
    // 06/26/2017 commented out ticks[OscEvent] = page.IhvLimitVins;
...
void prepareData(void){  
  ...
  //else if( reportFlashCheck ) prepareFlashCheck();
      ...
      *((INT16 *)(ptr+14)) = reportStatus;
      // *((INT16 *)(ptr+14)) = (INT16)page.VhvLimitVout;
      /*
    ...
    switch(reg){     
	  ...
	  page.fetDeltaCurrent = val;
        ...
        */
        ...
        case SetVhvLimitVout:
        case SetIhvLimitVout:
        case SetVhvLimitVins:
        case SetIhvLimitVins:
          ptr_int16 = &page.LowCurrentLimitTo255;
          ptr_int16[reg - SetLowCurrentLimitTo255] = val;
#ifdef MidString
