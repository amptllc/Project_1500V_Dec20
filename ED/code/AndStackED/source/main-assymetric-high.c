...
ParameterBlock page; // page is always 240 bytes long
...
void ov_startup(void){ if( page.ov_startup ){ T3CC0 = page.ov_startup<page.ov ? page.ov_startup : page.ov; *((INT16 *)0xF51E) = TICKS_IN_SEC; } P1_4 = 1; }
/******************************************************************************/
//INT16   last[ 8 ]; //Vout, lastPin, lastIout, lastGnd, lastRef, lastVin, lastTex, lastTin;
void read_page(void){
    mymemcpy( (BYTE *)&page, (BYTE *)PAGEADDR, PAGESIZE ); 
    // mymemcpy( (BYTE *)&page, (BYTE *)0x5C00, PAGESIZE ); 
    // if( ( page.barrier != 0xAAAA ) || ( page.post_barrier != 0x5555 ) ) 
      // mymemcpy( (BYTE *)&page, (BYTE *)0x400, PAGESIZE ); 
    //else 
    //  page.imageAddr = *((INT16 *)0x4D4);
...
//    P1_4 = 1;  P1_3 = 1; // MPP always on
    // P1_4 = page.module;  P1_3 = page.mpp; // 09/06/2017 module off in code 
    P1_4 = 0;  P1_3 = page.mpp; // 09/06/2017 Mike version

    if( resetReason  == 2 ){ setOC = page.oc; T3CC0 = page.ov; }
    else                   {
            #ifdef MidString
                T3CC1 = 0; setOC = page.oc;  
            #else    
                T3CC1 = page.oc; 
            #endif
//            ov_startup();
...
BYTE *ptr; BYTE *pptr = (BYTE *)&page;
    ...
    switch(reg){
        ...
        case MODULE: // Module on / off
            ...
            if( val && ((P1 & 0x18)==0) ){ P1_3 = 1; ov_startup(); /*P1 |= 0x18;*/  *((INT16 *)0xF50A) = TICKS_IN_SEC ;  } // bypass mode with delay          
            //else                         { if( val ) P1 |= 0x10; else P1 &= ~0x10; }
            //else                         { if( val ){ if( P1_4 == 0 ) ov_startup(); }else P1_4 = 0; }
            else                         { if( val ){ if( P1_4 == 0 ){ T3CC1 = 0; ov_startup(); } }else P1_4 = 0; }
        break;
        case SET_T3CH0:  /*if( !page.fuseOVOC )*/ T3CC0 = val;      break;
        case SET_T3CH1:  /*if( !page.fuseOVOC )*/ T3CC1 = val;      break;
        /*
        ...
        */
        case SET_CHANNEL: 
            ...
            if( !page.is500Always ) curChannel = CHANNR = val;  
        break;
        ...
        //case FUSE_OVOC:      page.fuseOVOC = 0;
            // go through
        case EIGHT_K_WRITEMFPB: main_pb = 1; // go through
        case FLASH_AVAILABLE: 
            ...
            // go through
        case COEFFICIENTS_AVAILABLE:     savePB();     break;
        case EIGHT_K_WRITEMPB:  main_pb = 1; savePB(); break;
        ...
        break;
        */
        case CALIBRATION_FROM_FLASH:   pptr = (BYTE *)0x400;        /* go through */
        case EIGHT_K_READMFPB: if( reg != CALIBRATION_FROM_FLASH ) pptr = (BYTE *)0x5C00;
        case REPORT_SCALING:   
          if( !reportScaling ){
                if( base_ptr==NULL ){
        ...
        case CHECK_FLASH:      reportFlashCheck = calibrationRepeat*8 + 1; /* 5*4-1 */;    break;
        ...
        case USE_12_kBod:      page.use12kbod = val;  break;

        case USE250:           page.use250kbod = val; //break;
        case IS_500_ALWAYS:    
        ...
        // win 7 bytes. 
        case SetCriticalLevel500:   case SetCriticalLevel10:
...
//             asm( "NOP" );   asm( "NOP" );             
        ...
        case CALIBRATION_REPEAT:
            if( val&0x1F  ) calibrationRepeat = val; else calibrationRepeat = 4;
        break;

        case REP_STEP:   if( val < 5 && val >50 )  break;  // else go through
        case NETWORK_CNST_1: case NETWORK_CNST_2: 
        case MAX_OFF:        case OC_STEP: 
            ptr = (BYTE *)&page.repStep; ptr[ reg-REP_STEP ] = val; break;
        case VIN_DISABLE_RADIO: case SET_RBUNCH: case SET_GBUNCH: case SHORT_CIRCUIT_LEVEL: //DEV250:
