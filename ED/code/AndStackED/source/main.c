...
// 06/27/2017 - Firmware Version now stored in code
// Pattern is HHHHLLLL-A_BC, where HHHH is the FWVersionHigh value,
// LLLL is the FWVersionLow value, A is FWSuffixOne value and
// BC are the FWSuffixTwo and FWSuffixThree values.  The _ and the
// - are implied.

static const UINT16  FWVersionHigh = 7000; // LBC need to place official version in code
static const UINT16  FWVersionLow = 11;
static const BYTE    FWSuffixOne = '1';
static const BYTE    FWSuffixTwo = 'B';
static const BYTE    FWSuffixThree = 'B';
...
static const BYTE ModuleStatusOff = 0xE7;
static const BYTE ModuleStatusOffFetFailure = 0x7E;
static const BYTE ModuleStatusCanTurnOn = 0x00;


/* 12/29/2017 BEGIN Protection global variables */

static const BYTE ModuleStatusOffProtection = 0x6A;
static const BYTE ModuleStatusOffTemperature = 0xA6;

static const BYTE NoProtectionsSetMask = 0x00;
static const BYTE VoutLimitMask = 0x040;
static const BYTE IoutLimitMask = 0x080;
static const BYTE VoutAndIoutLimitMask = 0xC0;
static const BYTE NotVoutAndIoutLimitMask = 0x3F;
static const BYTE NotVoutAndIoutLimitNoCountDownMask = 0x3E;

static const BYTE TemperatureModuleOffMask = 0x20;
static const BYTE TemperatureSteppedDownMask = 0x10;
static const BYTE TemperatureModuleOffAndSteppedDownMask = 0x30;
static const BYTE NotTemperatureModuleOffAndSteppedDownMask = 0xCF;

static const BYTE CountDownTimerActive = 0x01;
static const BYTE CountDownTimerNotActive = 0xFE;

/* variable to track protection settings */
static BYTE ProtectionLimitsExceeded = 0x00;

static const INT16 Volts_50 = 1666; /* 50 * 33.3333 as int16 */
static const INT16 Amps_1 = 1000;

/* 12/29/2017 END Protection global variables */

BOOL   enable2SendData  = FALSE,       txDisabled = FALSE,                    alignMPP = TRUE,
...
// 9th Line : fetFailureCount through IhvLimitVins (only LowCurrentLimitTo255 stayed same)
...
0x55, 0x55
 ...
 
};
...
// 06/26/2017 - Moved from local declarations in function checkVin() to const here
// old switchTo255Lim -> 60000, old switchFrom255Lim ->  57000 (1000V values shown)
// old shortCircuitProtectionOn -> 12000, old shortCircuitProtectionOff -> 16000 (1000V values shown)
// values used in checkVin (global to avoid declaration(s) every 120 MS)

// 07/05/2017 - switchTo255Lim was 43400, switchFrom255Lim was 40000
#ifdef MidString
const UINT16 switchTo255Lim = 44500,
    switchFrom255Lim = 42000, 
    shortCircuitProtectionOn = 10000, 
    shortCircuitProtectionOff = 13400; 
#endif
...
//static const float DEF_CURRENT_MULTYPLIER = 3.93; //7.93; //5.536;
...
//#endif
//  if( k_oc2timer < 1.0 ) k_oc2timer = DEF_CURRENT_MULTYPLIER / (page.linearK[2][0]-4.0);
// //  if( test4Zero((BYTE*)&k_oc2timer, 4) ) k_oc2timer = DEF_CURRENT_MULTYPLIER / page.linearK[2][0];
//  val = (UINT16)( oc * k_oc2timer + 0.5 );
//  if( val > 255 ) val = 255;
//  return (BYTE) val; 
//}

// 03/24/2017 static const float DEF_CURRENT_MULTYPLIER = 3.93; //7.93; //5.536;
...
UINT16 val;
  ...
  if( oc > ocShortCircuitLimit ) oc = ocShortCircuitLimit;  // Short Circuit protection.
#endif
  // 03/24/2017  if( k_oc2timer < 1.0 ) k_oc2timer = DEF_CURRENT_MULTYPLIER / (page.linearK[2][0]-4.0);
  // 03/24/2017 if( k_oc2timer < 1.0 ) k_oc2timer =  7.6 / (page.linearK[2][0]);
  
  if( k_oc2timer < 1.0 ) k_oc2timer =  7.93 / (page.linearK[2][0]);
  
//  if( test4Zero((BYTE*)&k_oc2timer, 4) ) k_oc2timer = DEF_CURRENT_MULTYPLIER / page.linearK[2][0];

  // 03/24/2017 added line
  if ( oc > 240 ) oc = 240;
  
  // 03/24/2017 val = (UINT16)( oc * k_oc2timer + 0.5 );
...
void read_page(void){
    ...
    page.versionHigh = FWVersionHigh;
    page.versionLow = FWVersionLow;
    page.versionSuffix[0] = FWSuffixOne;
    page.versionSuffix[1] = FWSuffixTwo;
    page.versionSuffix[2] = FWSuffixThree;    
    ch255 = FALSE;
}
...
void checkVin(void)
{
  ...
  /* 12/29/2017 BEGIN adding 1500V Protection Variables */
  static INT16 protectionModuleOffTimer = 0;
  /* 12/29/2017 END adding 1500V Protection Variables */
  ...
  if (checkForFETFailure)
  {
    ...
    if ( ((INT16)adcs16[2] > page.fetCurrentThreshold) && ( (iOutIin1Diff < page.fetDeltaCurrent) || (iOutIin2Diff < page.fetDeltaCurrent) )  )
    {
  ...
  /* 12/29/2017 BEGIN Adding 1500V Protection Code */
  
  if (fetFailure == FALSE) /* If fetFailure then module is already off */
  {
    /* Check if Vout has exceeded VoutLimit and has not yet been observed */
    if ((page.VoutLimitCount > 0) && ((ProtectionLimitsExceeded & VoutLimitMask) != VoutLimitMask) && (adcs16[0] > page.VoutLimit))
    {
      P1_4 = 0; /* turn Module off */
      P1_3 = 0; /* turn MPP off */
      
      *ModuleOffRememberedPtr = ModuleStatusOffProtection; /* Remember why module is off */
      ProtectionLimitsExceeded |= VoutLimitMask;   /* Set Protection byte to indicate Vout Protection */
      
      if (protectionModuleOffTimer < page.VoutLimitCount) /* Set Protection timer to appropriate value */
      {
        protectionModuleOffTimer = page.VoutLimitCount;
      }
    }
    
    /* Check if Iout has exceeded IoutLimit and has not yet been observed */
    if ((page.IoutLimitCount > 0) && ((ProtectionLimitsExceeded & IoutLimitMask) != IoutLimitMask) && ((INT16)adcs16[2] > page.IoutLimit))
    {
      P1_4 = 0; /* turn Module off */
      P1_3 = 0; /* turn MPP off */
      
      *ModuleOffRememberedPtr = ModuleStatusOffProtection; /* Remember why module is off */
      ProtectionLimitsExceeded |= IoutLimitMask;          /* Set Protection byte to indicate Vout Protection */
      
      if (protectionModuleOffTimer < page.IoutLimitCount) /* Set Protection timer to appropriate value */
      {
        protectionModuleOffTimer = page.IoutLimitCount;
      }
    }
    
    /* Is Vout or Iout protection indicated? */
    if (ProtectionLimitsExceeded >= VoutLimitMask)
    {
      ProtectionLimitsExceeded |= CountDownTimerActive; /* Set the Countdown timer bit "true" until proven otherwise */
      
      if ((ProtectionLimitsExceeded & IoutLimitMask) == IoutLimitMask) /* IoutLimit Protection has been recognized */
      {
        if ((INT16)adcs16[2] > (page.IoutLimit - Amps_1)) /* When Iout above IoutLimit - 1Amp, countdown timer is "re-maxed" */
        {
          ProtectionLimitsExceeded &= CountDownTimerNotActive; /* Countdown condition fails, so change timer bit */
          if (protectionModuleOffTimer < page.IoutLimitCount) /* Set Protection timer to appropriate value */
          {
            protectionModuleOffTimer = page.IoutLimitCount;
          }
        }
      } /* END if ((protectionLimitsExceeded & IoutLimitMask) == IoutLimitMask) */
      
      if ((ProtectionLimitsExceeded & VoutLimitMask) == VoutLimitMask) /* VoutLimit Protection has been recognized */
      {
        if (adcs16[0] > (page.VoutLimit - Volts_50)) /* When Vout above VoutLimit - 50 volts, countdown timer is "re-maxed" */
        {
          ProtectionLimitsExceeded &= CountDownTimerNotActive;  /* Countdown condition fails, so change timer bit */
          if (protectionModuleOffTimer < page.VoutLimitCount)   /* Set Protection timer to appropriate value */
          {
      ...
      } /* END if ((protectionLimitsExceeded & VoutLimitMask) == VoutLimitMask) */
      
      /* Is Countdown timer active? (meaning it is ok to decrement it) */
      if ((ProtectionLimitsExceeded & CountDownTimerActive) == CountDownTimerActive)
      {
        protectionModuleOffTimer -= 120;
        if (protectionModuleOffTimer <= 0)
        {
          protectionModuleOffTimer = 0;
          ProtectionLimitsExceeded &= NotVoutAndIoutLimitNoCountDownMask;
          if (ProtectionLimitsExceeded == 0)
          {
            *ModuleOffRememberedPtr = ModuleStatusCanTurnOn;
            P1_3 = page.mpp;
            ov_startup();   
          } /* END if (ProtectionLimitsExceeded == 0) */
          else /* Some form of Temperature Protection is on, act accordingly */
          {
            P1_3 = page.mpp;
            if ((ProtectionLimitsExceeded & TemperatureModuleOffMask) == 0x00)
            {
              *ModuleOffRememberedPtr = ModuleStatusCanTurnOn;
              ov_startup();                 
            }
            ...
            {
              *ModuleOffRememberedPtr = ModuleStatusOffTemperature;
            }
          }
        } /* END if (protectionModuleOffTimer <= 0) */
      } /* END if ((ProtectionLimitsExceeded & CountDownTimerActive) == CountDownTimerActive)*/
    } /* END if (ProtectionLimitsExceeded >= VoutLimitMask)*/
  } /* END if (fetFailure == FALSE) */
  
  /* 12/29/2017 END Adding 1500V Protection Code */
  ...
  if( (flags[ PackageReceived ]) || (flags[DataSent]) )
  {
  ...
  txDisabled = 0;
  if ( page.vin_disable_radio )
    txDisabled = ( safe_vin_16 < (1000*(UINT16)page.vin_disable_radio) );
...
#ifdef MidString
  /* 06/26/2017 - Added additional code regarding switching to and from channel 255, (looking at Iin1 (adcs16[6]) ) */
  if (!ch255)
  {
    if (safe_vin_16 > switchTo255Lim)
    {
      switchTo255();
      netValue = MainNetValueSafeVin;
    }
    else if ((INT16)adcs16[6] < (INT16)page.LowCurrentLimitTo255)
    {
      switchTo255();
      netValue = MainNetValueLowCurrentLimit;
    }
  }
  else if ( ch255 && (( safe_vin_16 < switchFrom255Lim ) && ((INT16)adcs[6] >= (INT16)page.LowCurrentLimitTo255)))
  {
    read_page();
    if( page.is500Always )
      curChannel = page.starting500Channel;
    softKick();
    netValue = MainNetValueStandard;
  }
#endif
  ...
  /* Start of Temperature protection code */
  _coeff = page.linearK[4];
  t =  (INT16) lin_coeff_t( ((float)last[6]) * checkVref );
  
  /* if T_SUPER_HOT enabled, and temperature exceeds value, turn off unit.
     Critical Temperature Protection, i.e., T_SUPER_HOT */
  if( page.t_mod_off && (t > ((INT16)page.t_mod_off)) )
  { 
    P1_4 = 0;
    if (*ModuleOffRememberedPtr == ModuleStatusCanTurnOn)
      *ModuleOffRememberedPtr = ModuleStatusOffTemperature;
    ProtectionLimitsExceeded |= TemperatureModuleOffAndSteppedDownMask;
  }
  else if ( page.fallback_time && page.fallback_time!=0xFF )
  { /* Temperature above T_HOT */
    if ( checkVoutOnT )
      checkVoutOnT--;
    else
    {
      checkVoutOnT = 8 * (UINT16)page.fallback_time;
      if ((ProtectionLimitsExceeded & TemperatureSteppedDownMask) == TemperatureSteppedDownMask)
      {
        if ( t < ((INT16)page.ton_fallback) ) /* if temperature less than T_COLD */
        {
          if ( !P1_4 )
          {
            if (*ModuleOffRememberedPtr == ModuleStatusOffTemperature)
            {
              *ModuleOffRememberedPtr = ModuleStatusCanTurnOn;
              ov_startup(); /* module attempt turn on , starting OV seq */
            }
          }
                      
          T3CC1 = oc2timer(theOC = setOC);

          ProtectionLimitsExceeded &= NotTemperatureModuleOffAndSteppedDownMask; /* Both Protections cancelled here */
        } /* END  if ( t < ((INT16)page.ton_fallback) )  */
        else if ( P1_4 && theOC )
          T3CC1 = oc2timer( --theOC );
      } /* END  if( isSteppedDown ) */
      else if ( P1_4 && ( t > ((INT16)page.toff_fallback)) )
      {
        if ( (theOC > page.oc_protection ) || (OcRamp <= 0))
        {
          atomicChange( OcRamp, 0 ); 
          theOC = page.oc_protection;
        }
        else
          theOC--;
        
        ProtectionLimitsExceeded |= TemperatureSteppedDownMask;
        T3CC1 = oc2timer( theOC );
      } /* END else if ( P1_4 && (t > ((INT16)page.toff_fallback)) ) */
    } /* END else (checkVoutOnT == 0) */
  } /* END else if (page,fallback_time ... */
  ...
  if ( page.shortCircuitLevel && P1_4 )
  { /* module on and shortCircuitLevel is not 0 */
    
    UINT16 vout;
    _coeff = page.linearK[0];
    vout = convU( lin_coeff( ((float) last[0])*checkVref ) );
    
    if ( ocShortCircuitLimit < 255 )
    { /* Short Circuit Protection is currently ON */
    ...
    {  /* Short Circuit Protection is currently OFF */
      if ( vout < shortCircuitProtectionOn ) // was 6000 - 120V on Midstring 600V, was 1200 for MS1000, turning Short Circuit Protection ON
      {
        ocShortCircuitLimit = page.shortCircuitLevel;
        T3CC1 = oc2timer( theOC );
      }
    }
  }
  else 
    ocShortCircuitLimit = 255;  // if module OFF -> no Short Circuit Protection, turning it OFF
#endif    
...
void main_loop(void)
{
  ...
  for ( idx = 0; idx < N_Of_Flags; idx ++ )
  {
    if( flags[ idx ] )
    {
      ...
      case DataSent:
        ...
        else if ( page.isRelay && (!page.is500Always) )
        { 
          setupRepeater( got_gw );
          receive();
        }
        break;
      ...
      case AltCheckVin:
        if ( ( PKTSTATUS & 0x4 ) && ( ( MARCSTATE > 0xC ) && ( MARCSTATE < 0x10 ) ) )
          flags[AltCheckVin] = TRUE; /* receving a packet */
        else if( flags[PackageReceived] || flags[DataSent] )
          flags[AltCheckVin] = TRUE; /* receving a packet */
        else if( ( ( ticks[SendData] > 14*TICKS_IN_MS ) || ( ticks[SendData] == 0 ) ) && ( ( ticks[DelayedPrepareData] > 14*TICKS_IN_MS ) || ( ticks[DelayedPrepareData] == 0 ) ) ) 
      ...
      case Cycle:
        if ( dec( &cycles2SoftKick )  )
        { 
          softKick();
          cycles2SoftKick  = page.defCycles2SoftKick;  
        }
        if ( cycles2HardKick < 10 )
        {
          switchTo255();
          ch255 = FALSE;
        }
                            
        if ( ++cyclesFromStart > 50 )
        ...
        if ( (!wasAHardResetOrPowerOn) && page.searchCommunication )
        {
          ...
          else
          {
            ...
            { 
              ...
              cycles2Step10Channel = 0;
              
              page.is500Always = TRUE;
              cyclesToStep500Channel = page.defCyclesToStep500Channel;
              
              if( page.starting500Channel == 0xFF )
                page.starting500Channel = page.channel;
  ...
  }/* END   for ( idx = 0; idx < N_Of_Flags; idx ++ ) */
  ...
  for ( idx = 0; idx < N_Of_Ticks; idx++ )
  {
    if ( ticks[ idx ] < 0 )
    {
      ...
      case SendData:
        ...
        *((BYTE *)0xF538) = 0;
        ...
        if ( packagePrepared )
        {
          ...
          if ( isCalibrationPackage && reportScaling )
          {
          ...
          {
            BYTE b;
            if ( page.rbunch && ( page.rbunch < page.gbunch ) )
              b = page.rbunch;
            else
              b = page.gbunch;
            ...
            {
              if ( b <= 18 )
                atomicChange( SendData, lastCycle*b - 1 -*((BYTE *)0xF538)-xored);
            }
            else
            {
              if ( b == 1 )
                atomicChange( SendData, lastCycle   - 1 -*((BYTE *)0xF538)-xored);
            }
            xored ^= 1;
          }
        }/* END if ( packagePrepared ) */
        break;
      ...
      case Init:
        ...
        {
          P1_3 = 1; 
          atomicChange( BypassMode, TICKS_IN_SEC ); // switching mpp on and switching it off (bypass mode) only after delay
        }
        else
          P1_3 = page.mpp;
        if( page.module )
      ...
      case Hop:
        if ( page.hoppingAllowed && !page.is500Always )
        {
          if ( si->radioMode == RADIO_MODE_TX  )
          {
            atomicChange( Hop, 1 );
            break;
          }
      ...
      case Ms:
        atomicChange( Ms, TICKS_IN_SEC );
        cycles2HardKick--;
        break;
      
      case Cycle: /* kicking and search logic */
        atomicChange( Cycle, lastCycle );
        atomicChange( ReceiveData, lastCycle - _slot - 5*TICKS_IN_MS );
        flags[ Cycle ] = TRUE;
        if (got_gw)
          got_gw--;
        if ( isOnTestStand )
        ...
        { /* most of the time is spent here*/
          *((BYTE *)0xF538) = 0;
          ...
          if ( page.gbunch )
          {
            ...
            {
              atomicChange( DelayedPrepareData, page.gbunch * lastCycle - *((BYTE *)0xF538) - 1 -xored);
            }
            else if ( !page.use12kbod && ( page.gbunch == 1 ) )
            {
              atomicChange( DelayedPrepareData, lastCycle - *((BYTE *)0xF538) - 1 -xored);
            }
          }
        }
        break;
  ...
  }/* END for ( idx = 0; idx < N_Of_Ticks; idx++ ) */
}
#define BARRIER  0xAA
void quartz();

void longTickWait( BYTE q )
{
  while( q-- )
    tickWait( 50*TICKS_IN_MS );
}
...
void main(void)
{
  ...
  *((BYTE *)0xFFFE) = 0x5A;
  ...
  for( i = 0; i<15; i++ )
  { // was 15 the self check !!!! check line 663 if loop limit changed !!!
    BYTE *ptr = (BYTE *)( 0x800 + 0x400*i );
    BOOL flag = TRUE;
    *((BYTE *)0xFFFF) = 0x10;
  ...
  case 2:  /* Watchdog Reset  */
    ...
    else if (*ModuleOffRememberedPtr == ModuleStatusOffProtection)
    {
      P1_4 = 0;
    }
    else if (*ModuleOffRememberedPtr == ModuleStatusOffTemperature)
    {
      P1_4 = 0;
    }
    else
  ...
  case 1: case 3: case 0: /* Power On Reset */
    *ModuleOffRememberedPtr = ModuleStatusCanTurnOn;
  ...
  if ( mymemcmp( (BYTE *)&cycles2SoftKick, (BYTE *)&page.defCycles2SoftKick, 4 ) )
    reset();
...
MainLoopLabel:
    ...
    { 
      ...
      if ( lastCycle == 0 )
        lastCycle = 840*TICKS_IN_MS;
      if ( ticks[ Cycle ] > lastCycle )
        ticks[ Cycle ] = lastCycle;
      if ( ticks[ Cycle ] == 0        )
        ticks[ Cycle ] = lastCycle;
      if ( cycles2SoftKick > page.defCycles2SoftKick )
        cycles2SoftKick = page.defCycles2SoftKick;
      if ( cycles2SoftKick == 0 )
        cycles2SoftKick = page.defCycles2SoftKick;
      if ( (cycles2HardKick&0xFF) == 0 )
        reset();
      if ( ticks[ Ms ] > TICKS_IN_SEC )
        ticks[ Ms ] = TICKS_IN_SEC;
      if ( ticks[ Ms ] == 0 )
        ticks[ Ms ] = TICKS_IN_SEC;
...
void readNextValue(void){ //Scheduled(void){  // 
  ...
  else
  {
    ...
    {
      BYTE idx; 
      INT16 grnd = adcsTemp[ 3 ];
      
      for( idx = 0; idx < N_CHANNELS; idx++ )
      {
        adcBuf[ idx ][ bufferCount ] = adcsTemp[ idx ];

        if( adcsTemp[ idx ] < grnd )
          last[idx] = 0; 
        else 
          last[idx] = adcsTemp[ idx ] - grnd;
      }
...
{  
  ...
  if ( reportScaling )
  { 
    if ( base_ptr==coefficients_buffer )
      prepareCoeff();
    else 
      prepareScaling();
    isCalibrationPackage = TRUE;
  }
  else if ( enable2SendData )
  {
    BYTE *ptr  = radioPktBuffer;
    computeADCs( FALSE, FALSE );
    
    mymemcpy( ptr, (BYTE *)&utc, 4 );
    ptr +=4;
    *ptr++ = (BYTE)curChannel;
    *ptr++ = ( ( page.netId&0xF | ( ( P1 & 0x18 ) << 1 ) ) << 2 );
    
    copyMac( ptr );
    ptr += 6; /* mac address is 6 bytes (12 characters) */
    /* At this point, 12 bytes gone */
    
    mymemcpy( ptr, (BYTE *)adcs16, 16 );
    
    /* 06/26/2017 To help debug new functionality in checkVin(), only add this values if reportStatus is 50 or below */
    if (reportStatus <= 50)
    {
      if (ProtectionLimitsExceeded != 0)
      {
        reportStatus = 180 + ProtectionLimitsExceeded;
      }
      else
      {
        if (page.is500Always )
          reportStatus += 2 + ((UINT16)CHANNR)*100;
        else if (page.isRelay  )
          reportStatus += 1 + page.repeaterChannel*100;
        if ( page.searchCommunication )
          reportStatus += 4;
        if ( page.use250kbod )
          reportStatus += 8;
        if ( isOnTestStand )
    ...
    *((INT16 *)(ptr+14)) = reportStatus;
    ...
    ptr += 16;
    *ptr++ = lastRssi ? ((BYTE) theRealOffset) : 0x3F;
    ...
    *ptr++ = (BYTE) ( lastRssi );    // 29
    ...
    if( P1_4 )
    {
      ...
      *ptr   = (BYTE)   theOC;
#ifdef MidString
      if( *ptr > ocShortCircuitLimit )
        *ptr = ocShortCircuitLimit;
#endif
    }
    else
    {
...
void quartz(void)
{
  ...
  while(!XOSC_STABLE && counter-- )
    halWait(2);      // waiting until the oscillator is stable
...
BYTE *ptr; BYTE *pptr = (BYTE *)&page;
    ...
    switch(reg){
        case MPP:  /* MPP on (1) / off (0) */
            if( val > 1 )
              break;
            if ((ProtectionLimitsExceeded & VoutAndIoutLimitMask) != 0)
              break;
            if( val )
              P1_3 = 1;
            else 
        ...
        case MODULE: /* Module on (1) / off (0) */
            ...
            if (val == 1) // If Module on, then set FET failure check appropriately
            {
              *ModuleOffRememberedPtr = ModuleStatusCanTurnOn; // Module On command resets this
              ProtectionLimitsExceeded = NoProtectionsSetMask;
            ...
            { 
              P1_3 = 1; 
              ov_startup(); /*P1 |= 0x18;*/ 
              atomicChange( BypassMode, TICKS_IN_SEC );
            } // bypass mode with delay
            else 
            { 
              if( val )
              { 
...
//        case PRODUCTION:       page.production = val;                   break;
        ...
        */
        ...
        // 06/27/2017 commented out case TEST_1:  case TEST_2:   case TEST_3:   case TEST_4:
    ...
    switch(reg){     
	  ...
	  page.fetDeltaCurrent = val;
        ...
        */
        ...
        case SetImageAddr:
        // 06/27/2017 Version no longer settable case SetVersionLow:                     case SetVersionHigh:
        //default:
        ...
        case SetLowCurrentLimitTo255:
          page.LowCurrentLimitTo255 = (INT16)val;
          break;
        case SetVoutLimitCount:
          page.VoutLimitCount = (INT16)val;
          break;
        case SetVoutLimit:
          page.VoutLimit = (UINT16)val;
          break;
        case SetIoutLimitCount:
          page.IoutLimitCount = (INT16)val;
          break;
        case SetIoutLimit:
          page.IoutLimit = (INT16)val;
          break;
          // 01/09/2017 ptr_int16 = &page.LowCurrentLimitTo255;
          // 01/09/2017 ptr_int16[reg - SetLowCurrentLimitTo255] = val;
          /*
#ifdef MidString
          ocLimit1 = FALSE;  // Ensure new values are used in checkVin (in case either ocLimit is TRUE)
          ocLimit2 = FALSE;  // 06/29/2017 - changes (2 lines)
#endif
          */
