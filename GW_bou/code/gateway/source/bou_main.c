...
extern BYTE radioPktBufferRx[PACKET_LENGTH + 3]; // to accomodate maximum possible package PACKET_LENGTH_ED + 3];     // Buffer for packets to send or receive, sized to match the receiver's needs
extern BYTE radioPktBuffer  [4][PACKET_LENGTH + 3];  // Buffer for packets to send or receive, sized to match the receiver's needs
extern BYTE radioPktBufferTx[PACKET_LENGTH + 3];     // Buffer for packets to send or receive, sized to match the receiver's needs
...
void setupTimer1(void){
    ...
    SET_WORD(T1CC0H, T1CC0L, 1500 - 1);  // 500 mks per tick
