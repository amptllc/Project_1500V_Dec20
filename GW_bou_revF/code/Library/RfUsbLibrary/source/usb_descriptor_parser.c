...
*/
void usbdpInit(void) {  usbdpData.pDesc = (const BYTE __xdata *) pUsbDescStart;  } // usbdpInit
...
* \return	 A pointer to the wanted descriptor type, or \c NULL if it was not found.
*/
void __xdata* usbdpFindNext(BYTE wantedType, BYTE haltAtType) {
   void __xdata *pResult;
   pResult = NULL;
   // As long as we haven't reached the end...
   while (usbdpData.pDesc != (void __xdata *) pUsbDescEnd) {
...
* \return	 A pointer to the \ref USB_DEVICE_DESCRIPTOR, or \c NULL if it was not found.
*/
...
* setting.
* \note It is not necessary to call \ref usbdpInit() before this function.
* \param[in]       cfgValue     The configuration value (\ref USB_CONFIGURATION_DESCRIPTOR.bConfigurationValue)
* \param[in]       intNumber    The interface number (\ref USB_INTERFACE_DESCRIPTOR.bInterfaceNumber)
* \param[in]       altSetting   The alternate setting (\ref USB_INTERFACE_DESCRIPTOR.bAlternateSetting)
* \return		 A pointer to the \ref USB_INTERFACE_DESCRIPTOR, or \c NULL if it was not found.
*/
