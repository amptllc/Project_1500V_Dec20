...
typedef struct { 
  ...
  UINT16 VhvLimitVout               __attribute__((__packed__)) __attribute__ ((aligned (1)));// 2 136   (138)   
  UINT16 IhvLimitVout        __attribute__((__packed__)) __attribute__ ((aligned (1)));// 2 138   (140)   
  UINT16 VhvLimitVins     __attribute__((__packed__)) __attribute__ ((aligned (1)));//2  140   (142)  
  UINT16 IhvLimitVins __attribute__((__packed__)) __attribute__ ((aligned (1)));//  2  142   (144)
...
BOOL logParameterBlockInformation( char *filename, ParameterBlock page)
{
  ...
  if (f == NULL)
  ...
  else
    {
      ...
      fprintf( f, "Oscillation Params: LowCurrentLimitTo255 %d VhvLimitVout %d IhvLimitVout %d\r\nVhvLimitVins %d IhvLimitVins %d\r\n",
...
void usage( char * argv[] )
{
  ...
  printf("-VhvLimitVout  <optional,int> set value for integer register 20.\n");
  printf("-IhvLimitVout <optional, int> set value for integer register 21.\n");
  printf("-VhvLimitVins <optional, int> set value for integer register 22.\n");
  printf("-IhvLimitVins <optional, int> set value for integer register 23.\n");
...
int main(int argc, char **argv)
{
  ...
  UINT16 VhvLimitVout = 33333;        // raw ADC counts 
  INT16  IhvLimitVout = 200;          // oc setting 
  INT16  VhvLimitVins = 36667;        // raw ADC counts 
  INT16  IhvLimitVins = 40;           //  oc setting 

  INT16  LowCurrentLimitTo2558K = -250;  
  UINT16 VhvLimitVout8K = 33333; 
  INT16  IhvLimitVout8K = 200;   
  INT16  VhvLimitVins8K = 36667; 
  INT16  IhvLimitVins8K = 40; 
  ...
  for( i = 1; i < argc && retValue == OK; i++ )
	...
	{ 
      ...
      else if( strcmp( argv[i], "-VhvLimitVout" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-VhvLimitVout")) == OK)
	    {
	      VhvLimitVout = 0xFFFF & (int) val; // raw ADC counts,
	    }
	} 
      else if( strcmp( argv[i], "-VhvLimitVout8K" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-VhvLimitVout8K")) == OK)
	    {
	      VhvLimitVout8K = 0xFFFF & (int) val; // raw ADC counts,
	    }
	} 
      else if( strcmp( argv[i], "-IhvLimitVout" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-IhvLimitVout")) == OK)
	    {
	      IhvLimitVout =  0xFFFF & (int)val;  
	    }
	}
      else if( strcmp( argv[i], "-IhvLimitVout8K" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-IhvLimitVout8K")) == OK)
	    {
	      IhvLimitVout8K =  0xFFFF & (int)val;  
	    }
	}
      else if( strcmp( argv[i], "-VhvLimitVins" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-VhvLimitVins")) == OK)
	    {
	      VhvLimitVins = 0xFFFF & (int)val;   
	    }
	}
      else if( strcmp( argv[i], "-VhvLimitVins8K" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-VhvLimitVins8K")) == OK)
	    {
	      VhvLimitVins8K = 0xFFFF & (int)val;   
	    }
	}
      else if( strcmp( argv[i], "-IhvLimitVins" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-IhvLimitVins")) == OK)
	    {
	      IhvLimitVins =  0xFFFF & (int)val; 
	    }
	}        
      else if( strcmp( argv[i], "-IhvLimitVins8K" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-IhvLimitVins8K")) == OK)
	    {
	      IhvLimitVins8K =  0xFFFF & (int)val; 
	    }
	}        
      ...
      if( !is_gw )
		{
		  ...
		  if ((mod < 0) || (mod > 1))
		  ...
		  else
			{
			  ...
			  page.VhvLimitVout = VhvLimitVout;      // in raw adc V counts (Vout compared, 1/3 displayed value) 
			  page.IhvLimitVout = IhvLimitVout;      // new oc setting if VhvLimitVout indicates oc should change 
			  page.VhvLimitVins = VhvLimitVins;      // in raw adc V counts (Vins compared, 1/3 displayed value) 
			  page.IhvLimitVins = IhvLimitVins;      // new oc setting if VhvLimitVins indicate oc should change 
