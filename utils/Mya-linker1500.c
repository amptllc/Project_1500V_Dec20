...
typedef struct { 
  ...
  INT16 LowCurrentLimitTo255   __attribute__((__packed__)) __attribute__ ((aligned (1)));// 2 134   (136)   
  INT16 VoutLimitCount               __attribute__((__packed__)) __attribute__ ((aligned (1)));// 2 136   (138)   
  UINT16 VoutLimit        __attribute__((__packed__)) __attribute__ ((aligned (1)));// 2 138   (140)   
  INT16 IoutLimitCount     __attribute__((__packed__)) __attribute__ ((aligned (1)));//2  140   (142)  
  INT16 IoutLimit __attribute__((__packed__)) __attribute__ ((aligned (1)));//  2  142   (144)
...
ParameterBlock page8K __attribute__((__packed__)) __attribute__ ((aligned (1))) = {// 8K Parameter Block
  ...
  { 0, 0, 0, 0, 0, 0 }, // myMac
  {
    ...
    -0.000693, 0.0556, 0.613, -190 // -200 // -240 //+ 300.0
  ...
  // 01/09/2018 { {18.45, -2300}, {16.73, 0.0}, {7.45, -548.0}, {16.63, -60.0},  {0.435, 625.0}, {7.46, -558 }, {7.61, -511} },
   { {27.6, -3500}, {25.0, 0.0}, {8.4, -650.0}, {24.75, -235.0},  {0.435, 625.0}, {8.3, -680 }, {8.3, -685} },
  ...
  20000,   // VoutLimitCount (milliseconds in protection mode due to Vout > VoutLimit), 0 disables 
  46666,    // VoutLimit  threshold setting (Volts) to engage Over Voltage protection.
  23000,   // IoutLimitCount (milliseconds in protection mode due to Iout > IoutLimit), 0 disables
  20000,    // IoutLimit -> threshold setting (milliAmps) to engage Over Current protection. 
  { 0, 0, 0, 0 },  //versions[3], tests
  ...
  0, // rbunch
  0, // gbunch 
  100,  // 01/09/2018 170 // shortCircuitProtection
  0xFF, // reserved        
  120,  // sync_phase 
  128,  // synch_freq
  0x4D, // bandwidth250
  ...
  // LBC 10/22/16 (BYTE)0x89, // flags2
  (BYTE)0x81, // flags2 -> want stay_in_rx to be false by default
  0xFFFF, // prepAddr
...
ParameterBlock page __attribute__((__packed__)) __attribute__ ((aligned (1))) = {// Main image PB 
  ...
  23000,   // IoutLimitCount (milliseconds in protection mode due to Iout > IoutLimit), 0 disables
  20000,    // IoutLimit -> threshold setting (milliAmps) to engage Over Current protection.  
  { 0, 0, 0, 0 },  //version[3], tests in fourth position.
  0xFFFFFFFF,   //install date
  0xFF, 0xFF,   // k_ov_volt, k_oc_cur  
  0xF0F0, // syncword
  0, // vin_disable_radio
...
//float dfk[] = {-0.000693, 0.0556, 0.613, -240}; // new dFk[3] value
float dfk[] = {-0.000693, 0.0556, 0.613, -200}; // new dFk[3] value
...
INT16 roundFloat(float x)
{
  assert(x >= SHRT_MIN-0.5);
  assert(x <= SHRT_MAX+0.5);
  if (x >= 0.0)
    return (INT16) (x+0.5);
  return (INT16) (x-0.5);
}
      ...
      for( f1 = fopen( bootSec, "r+" ); f1 && !feof( f1 ); ret = fgets( buf, 64, f1 ) )
	{
	  if( buf[8] == '5' )
	      ...
	      for( j = 0; j < 2; j++, addr += 0x10 )
		{
	      ...
	      *((UINT16 *)tmp) = 0xAAAA;
	      // 07/06/2017 -> hard coded ov oc limits next linetmp[12] = page.ov;          tmp[13] = page.oc;
	      tmp[12] = 90;          tmp[13] = 100;
...
BOOL logParameterBlockInformation( char *filename, ParameterBlock page)
{
  ...
  if (f == NULL)
  ...
  else
    {
      ...
      fprintf( f, "Oscillation Params: LowCurrentLimitTo255 %d VoutLimitCount %d VoutLimit %d\r\nIoutLimitCount %d IoutLimit %d\r\n",
	       page.LowCurrentLimitTo255,  
	       page.VoutLimitCount, 
	       page.VoutLimit, 
	       page.IoutLimitCount, 
	       page.IoutLimit);
      ...
      fprintf( f, "Repeater %s Slave %s Searching %s 12kbod %s LastByteAsChannel %s Hopping %s WatchDogReset %s Production %s\r\n",
			   ...
			   page.flags&0x2 ? "YES" : "NO", page.flags&0x1 ? "YES" : "NO");
      
      fprintf( f, "ReportUTC %s fuseComm %s fuseOVOC %s useFEC %s stayInRX %s Speculative %s 250_kbod %s tryOtherImage %s\r\n",
			   page.flags2&0x80 ? "YES" : "NO", page.flags2&0x40 ? "YES" : "NO", page.flags2&0x20 ? "YES" : "NO",
			   page.flags2&0x10  ? "YES" : "NO", page.flags2&0x8  ? "YES" : "NO", page.flags2&0x4 ? "YES" : "NO",
			   page.flags2&0x2 ? "YES" : "NO", page.flags2&0x1 ? "YES" : "NO");
      ...
      fprintf( f, "shunt %d max-off %d syncword 0x%x \r\n",   
...
static const int ERROR_BAD_BOARD_PART_NUMBER = 21;
static const int ERROR_SN_LENGTH = 22;
...
void usage( char * argv[] )
{
  ...
  printf("purpose : To place the boot, main (low), and 8K (high) images into one hex file and\n");
  printf("set the default main and 8K image parameter blocks for Midstring V1300 optimizers.\n\n");
  printf("arguments\n");
  printf("-help <optional, no arg> print out this message and exit.\n");
  printf("-sku  <required,UINT32 (8,11,13  chars)> set SKU in Parameter Blocks .\n");
  ...
  printf("-LowCurrentLimitTo255  <optional,int> set value for integer register 19.\n");
  printf("-VoutLimitCount  <optional,int> set value for integer register 33, time in milliseconds for Over Voltage Protection.\n");
  printf("-VoutLimit <optional, int> set value for integer register 34, if Vout exceeds this value then go into Over Voltage Protection.\n");
  printf("-IoutLimitCount <optional, int> set value for integer register 35, time in milliseconds for Over Current Protection.\n");
  printf("-IoutLimit <optional, int> set value for integer register 36, if Iout exceeds this value then go into Over Current Protection.\n");
...
int convertSKU_ifDecimal(char * productID, 
			   UINT32 * firstPartOfSKU, 
			   UINT32 * secondPartOfSKU)
{
  int retValue = ERROR_BAD_BOARD_PART_NUMBER; // Set to OK if good
  long productIDLength = strlen(productID);
  const long int expectedShortLength = 8; // XXXXXXXX 
  const long int expectedLength = 11; // XXXXXXXX-XX 
  const long int expectedLongLength = 13; // XXXXXXXX-XXXX 
  long int val;

  if ((productIDLength == expectedLength) || (productIDLength == expectedLongLength))
    {
	  char * firstPart = NULL;
	  char * secondPart = NULL;
	  firstPart =  (char *)malloc(9);
	  secondPart = (char *)malloc(5);
	  memset(firstPart,0,9);
	  memset(secondPart,0,5);
	  strncpy(firstPart,productID,8);
	  strncpy(secondPart,&productID[9],productIDLength - 9);
	  if ((retValue = myStrToL(firstPart, &val, "-sku")) == OK)
	    {
	      *firstPartOfSKU = (UINT32)val;
		  val = 0;
		  if ((retValue = myStrToL(secondPart, &val, "-sku")) == OK)
			{
			  *secondPartOfSKU = (UINT32)val;

			  printf("%s:%d Converted SKU %d:%d\n",__FILE__,__LINE__, *firstPartOfSKU,*secondPartOfSKU);
			  fprintf(log,"%s:%d Converted SKU %d:%d\n",__FILE__,__LINE__, *firstPartOfSKU,*secondPartOfSKU);
			}
		  else
			{
			  printf("%s:%d ERROR: failed to convert second part of SKU:%s\n",__FILE__,__LINE__,productID);
			  fprintf(log,"%s:%d ERROR: failed to convert second part of SKU:%s\n",__FILE__,__LINE__,productID);
			}
	    }
	  else
		{
		  fprintf(log,"%s:%d ERROR: failed to convert first part of SKU:%s\n",__FILE__,__LINE__,productID);
		}

	}
  else if (productIDLength == expectedShortLength)
    {
	  if ((retValue = myStrToL(productID, &val, "-sku")) == OK)
	    {
	      *firstPartOfSKU = (UINT32)val;
		  *secondPartOfSKU = 0;
		...
		{
		  printf("%s:%d ERROR: failed to convert SKU:%s\n",__FILE__,__LINE__,productID);
		  fprintf(log,"%s:%d ERROR: failed to convert SKU:%s\n",__FILE__,__LINE__,productID);
		}
	}
  else
    {
      printf("%s:%d ERROR: length of productID (%s) is %ld, expect 8, 11, or 13\n", __FILE__,__LINE__,productID,strlen(productID));
      fprintf(log,"%s:%d ERROR: length of productID (%s) is %ld, expect 8, 11, or 13\n", __FILE__,__LINE__,productID,strlen(productID));
    }
  return retValue;
}
...
{
  char logFileName[] = "Mya-linker1300Log.txt";
  ...
  INT16  LowCurrentLimitTo255 = -250;    // mA,  
  UINT16 VoutLimitCount = 20000;         // milliseconds to stay in protection mode (0 - 32767)  
  INT16  VoutLimit = 46666;               // Vout Limit (Volts) if exceeded will trigger protection mode 
  INT16  IoutLimitCount = 23000;         // milliseconds to stay in protection mode (0 - 32767)
  INT16  IoutLimit = 20000;              // Iout Limit (milliAmps) if exceeded will trigger protection mode

  INT16  LowCurrentLimitTo2558K = -250;  // Protection mode registers not used  in 8K, same default settings as main image  
  UINT16 VoutLimitCount8K = 20000; 
  INT16  VoutLimit8K = 46666;   
  INT16  IoutLimitCount8K = 23000; 
  INT16  IoutLimit8K = 20000; 

  UINT32 firstPartOfSKU = 0;   // Actually this is the board number
  UINT32 secondPartOfSKU = 0;  // Actually this is the board number
  ...
  for( i = 1; i < argc && retValue == OK; i++ )
	...
	{ 
      ...
      else if( strcmp( argv[ i ], "-LowCurrentLimitTo255" ) == 0 )
	{ 
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-LowCurrentLimitTo255")) == OK)
	    {
	      LowCurrentLimitTo255=  0x7FFF & (int)val; 
	    }
	}
      else if( strcmp( argv[ i ], "-LowCurrentLimitTo2558K" ) == 0 )
	{ 
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-LowCurrentLimitTo2558K")) == OK)
	    {
	      LowCurrentLimitTo2558K = 0x7FFF & (int)val; 
	    }
	}
      else if( strcmp( argv[i], "-VoutLimitCount" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-VoutLimitCount")) == OK)
	    {
	      VoutLimitCount = 0x7FFF & (int) val; // milliseconds in protection mode 
	    }
	} 
      else if( strcmp( argv[i], "-VoutLimitCount8K" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-VoutLimitCount8K")) == OK)
	    {
	      VoutLimitCount8K = 0x7FFF & (int) val; // milliseconds in protection mode
	    }
	} 
      else if( strcmp( argv[i], "-VoutLimit" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-VoutLimit")) == OK)
	    {
	      VoutLimit =  0xFFFF & (int)val;  
	    }
	}
      else if( strcmp( argv[i], "-VoutLimit8K" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-VoutLimit8K")) == OK)
	    {
	      VoutLimit8K =  0xFFFF & (int)val;  
	    }
	}
	  else if( strcmp( argv[i], "-IoutLimitCount" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-IoutLimitCount")) == OK)
	    {
	      IoutLimitCount = 0x7FFF & (int)val;   
	    }
	}
      else if( strcmp( argv[i], "-IoutLimitCount8K" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-IoutLimitCount8K")) == OK)
	    {
	      IoutLimitCount8K = 0x7FFF & (int)val;   
	    }
	}
      else if( strcmp( argv[i], "-IoutLimit" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-IoutLimit")) == OK)
	    {
	      IoutLimit =  0x7FFF & (int)val; 
	    }
	}        
      else if( strcmp( argv[i], "-IoutLimit8K" ) == 0 )
	{
	  i++;
	  long val;

	  if ((retValue = myStrToL(argv[i], &val, "-IoutLimit8K")) == OK)
	    {
	      IoutLimit8K =  0x7FFF & (int)val; 
	    }
	}        
	...
	{ 
	  i++;
	  retValue = convertSKU_ifDecimal(argv[i],&firstPartOfSKU,&secondPartOfSKU);
	}
      else
	{
  ...
  if ((versionLowMain == 0) || (versionHighMain == 0))
    ...
    else if (strlen(sn) != 11)
     {
    printf("%s:%d ERROR The serial number length must be 11 characters.\n",
	     __FILE__,__LINE__);
      fprintf(log, "%s:%d ERROR The serial number length must be 11 characters.\n",
	     __FILE__,__LINE__);
      retValue = ERROR_SN_LENGTH;
     }
      ...
      if( !is_gw )
		{
		  ...
		  if ((mod < 0) || (mod > 1))
		  ...
		  else
			{
			  ...
			  // We will always base pureOffset on what the parameter block dFk[3] value is,
			  // no matter if dfk3 is default value or overridden in linker invocation.
			  // pureOffset is used only in 8K image, "equivalent" to dFk[3].
			  page.pureOffset = roundFloat(page.dFk[3]) + 36; // 36 is supplied by Anatoli, based on dFk polynomial
			  page8K.pureOffset = roundFloat(page8K.dFk[3]) + 36; // 36 is supplied by Anatoli, based on dFk polynomial
			  ...
			  page.LowCurrentLimitTo255 = LowCurrentLimitTo255;  // mA,  
			  page.VoutLimitCount = VoutLimitCount;      // If not zero, minimum time to remain in MODULE/MPP off state when Vout > VoutLimit  
			  page.VoutLimit = VoutLimit;                // If VoutLimitCount is not zero, if Vout > VoutLimit then go into protection mode.
			  page.IoutLimitCount = IoutLimitCount;      // If not zero, minimum time to remain in MODULE/MPP off state when Iout > IoutLimit
			  page.IoutLimit = IoutLimit;                // If IoutLimitCount is not zero, if Iout > IoutLimit then go into protection mode.
			  ...
			  page.utcLow = secondPartOfSKU;
			  page8K.utcLow = secondPartOfSKU;
				  ...
				  if( (retValue == OK) && additionalOutput ) 
					{
					  if ( copy( additionalOutput, 
						...
						{
						  printf("%s:%d INFO: copy of additional output finished successfully.\n",
								 __FILE__, __LINE__);
						  fprintf(log,"%s:%d INFO: copy of additional output finished successfully.\n",
								  __FILE__, __LINE__);
						}
					}
				}
